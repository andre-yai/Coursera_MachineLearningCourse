...
g2 = sigmoid(a2);
a3 = [ones(m,1),g2]* Theta2';
g3 = sigmoid(a3);

cost =  -Y .* log(g3) - (1-Y) .* log(1-g3);
...
regularization_parameters = (sum(sum(Theta1(:,2:end).^2)) + sum(sum(Theta2(:,2:end).^2)));
regularization = (lambda/(2*m))*(regularization_parameters);
